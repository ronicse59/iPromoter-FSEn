...
# Project version Anaconda 3.6.3
import methods
import authors
...
pipe2 = Pipeline([
               ...
               ('clf', SVC(kernel='poly', C=1, probability=True, decision_function_shape='ovo', tol=0.1, cache_size=200))])
pipe3 = Pipeline([
               ('sel', method_ensemble.ColumnSelector(c)),
               #('clf', LogisticRegression(random_state=0, n_jobs=1000))])
               ('clf', SVC(kernel='sigmoid', C=1, probability=True, decision_function_shape='ovo', tol=0.1, cache_size=200))])
